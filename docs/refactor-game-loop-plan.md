...
Boundary classification: `presentation`, focused on render snapshot runtime
wiring.

Why this cut:

`gameLoop.js` was still directly creating the base render snapshot runtime and
the render snapshot completion runtime, including construction model sync
callbacks, repair-box highlight sync, Squirtle assembly scene-object wiring,
debug-collider collection and tree-revival billboard completion. Those are
render snapshot presentation dependencies, while the game loop only needs the
two runtime handles and keeps their update order.

Moved out of `gameLoop.js`:

- direct imports for `createBaseRenderSnapshotFrameRuntime(...)` and
  `createRenderSnapshotCompletionFrameRuntime(...)`;
- construction presentation callback mapping for render snapshot preparation;
- repair-box highlight sync mapping;
- Squirtle assembly scene-object callback mapping;
- completion runtime wiring for debug colliders and tree-revival billboards.

Kept in `gameLoop.js`:

- high-level creation through `createRenderSnapshotRuntimeBundle(...)`;
- existing update calls and frame order for base and completion render snapshot
  runtimes;
- existing render snapshot data shape.

Line-count impact:

- Before this cut, committed `app/runtime/gameLoop.js` was `1951` lines.
- After this cut, `app/runtime/gameLoop.js` is `1944` lines.
...
The first run failed as expected because
`app/runtime/presentation/renderSnapshotRuntimeBundle.js` did not exist. After
adding the presentation-domain factory and integrating it into `gameLoop.js`,
the focused tests passed.

Passed:

```sh
npm test -- --run tests/renderSnapshotRuntimeBundle.test.js tests/baseRenderSnapshotFrame.test.js tests/renderSnapshotContext.test.js
git diff --check -- app/runtime/gameLoop.js app/runtime/presentation/renderSnapshotRuntimeBundle.js tests/renderSnapshotRuntimeBundle.test.js docs/refactor-game-loop-plan.md
npm run build
```
...
`npm test` completed with `2004` passed and `4` failed:
...
Boundary classification: `frame lifecycle`, focused on the early frame context
that combines gameplay opening state, placement preview state, placement camera
assist and blocker policy.

Study path:

1. `startGameLoop()` still wires the frame runtime as composition root.
2. `frame(now)` still decides when the gameplay frame context is calculated.
3. `gameLoopFrameRuntime.beginGameplayFrameContext(...)` now owns the local
   sequencing of opening `beginFrame`, opening camera/movement locks,
   placement preview detection, placement camera assist update, foundation
   build-zone camera focus and blocker resolution.
4. `requestAnimationFrame(frame)` remains in `gameLoop.js`.

Removed from `gameLoop.js`:

- local `beginGameplayFrameContext(...)`;
- direct use of `resolveGameLoopBlockers(...)` inside `gameLoop.js`;
- inline opening/placement/foundation blocker context composition.

Kept in `gameLoop.js`:

- temporal placement of this context immediately after scene sync;
- dependency wiring for placement contracts, opening runtime and camera assist;
- foundation build-zone camera focus implementation, because it still owns
  camera pose transition details and should be extracted separately if needed.

Line-count impact:

- Before this cut, committed `app/runtime/gameLoop.js` was `2494` lines.
- After this cut, `app/runtime/gameLoop.js` is `2471` lines.
- No new file was created; the boundary was added to the existing
  `gameLoopFrameRuntime.js` module.
...
The first run failed because `beginGameplayFrameContext(...)` did not exist
yet. After adding the method, the focused test passed.

Passed:

```sh
npm test -- --run tests/gameLoopFrameRuntime.test.js tests/gameLoopFramePolicies.test.js
git diff --check
npm run build
```
...
`npm test` completed with `1976` passed and `4` failed:
